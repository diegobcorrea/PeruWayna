...
if (strcmp ($res, "VERIFIED") == 0) {
    ...
    $txnID          = $_POST['txn_id'];
    $id_student		= $_POST['custom'];
    if($_POST['quantity'] == 1):
    	$minutes 		= (($_POST['mc_gross']/16) * 60);	
    else:
    	$minutes 		= ($_POST['quantity'] * 60);
   	endif;
     
    if($_POST['quantity']):
    	$user = $wpdb->get_row( "SELECT * FROM wp_bs_student WHERE id_student = '$id_student'", OBJECT );

		$newBalance = $user->hours_in_balance + $minutes;
		$update = $wpdb->query("UPDATE wp_bs_student SET hours_in_balance = $newBalance WHERE id_student = '$user->id_student'");
		$_SESSION['hours_in_balance'] = $newBalance;
	endif;

	$wpdb->insert('wp_bs_orders',
